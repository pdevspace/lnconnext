...
import { Event, Location, EventSection, Bitcoiner } from '@/types/event';
...
export function EventComponent({ event }: EventComponentProps) {
  ...
  const generateCalendarUrl = (event: Event) => {
    ...
    const endDate = event.endDate ? new Date(event.endDate).toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z' : startDate;
    const title = encodeURIComponent(event.name);
    const details = encodeURIComponent(event.description);
    const location = event.location ? encodeURIComponent(`${event.location.buildingName}, ${event.location.city}`) : '';
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="hidden lg:grid lg:grid-cols-12 lg:gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <EventImage event={event} activeImageIndex={activeImageIndex} onImageChange={setActiveImageIndex} />
          <EventActionButtons 
            event={event} 
...
function EventImage({ event, activeImageIndex, onImageChange }: EventImageProps) {
...
function EventActionButtons({ event, onAddToCalendar, onRegister, onShare }: EventActionButtonsProps) {
  return (
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Organizer details will be shown here</p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {event.sections.map((section) => (
          <div key={section.id} className="border-l-4 border-blue-500 pl-4">
            ...
            {section.spot && (
              <p className="text-sm text-muted-foreground mt-1">📍 {section.spot}</p>
            )}
            {section.description && (
