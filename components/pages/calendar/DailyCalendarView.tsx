...
const DailyCalendarView = ({ dailyList, filteredEvents, selectDate, setSelectDate }: DailyCalendarViewProps) => {
  return (
    <Card>
      <CardContent className="p-0">
        <div className="divide-y">
          {dailyList.map((date) => {
            const dayEvents = filteredEvents.filter((ev) => isSameDay(ev.startDate, date));
            ...
            return (
              ...
              >
                ...
                {isTodayFn(date) && (
                  <span className="ml-2 px-2 py-1 rounded bg-blue-100 text-blue-700 text-xs">Today</span>
                )}
              </div>
            );
          })}
